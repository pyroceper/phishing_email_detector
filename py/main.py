...
def predict_email():
    try:
        ...
        if db.check_save_data('./cache/p_temp.dat', email) == True:
            ...
            dt.predict(email)
            svm.predict(email)
            #predict_sentiment(email, bert, tokenizer, device) #bert
...
if __name__ == '__main__':
    ...
    #bert = BERTClassifier('bert-base-uncased', 2).to(device)
    #bert.load_state_dict(torch.load('./cache/bert_classifier.pth'))
    #bert.eval()
